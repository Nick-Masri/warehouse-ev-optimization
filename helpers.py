...
def initSolarPowModel(season, numDays, mornings, noons, nights):
    ...
    DN_solar_irr = pd.read_excel('DNIdata2020PA.xslx', sheet_name='DNIdata2020PA', usecols=[6], skiprows=2)

    DN_solar_irr = DN_solar_irr.to_numpy()
    DH_solar_irr = pd.read_excel('DNIdata2020PA.xslx', sheet_name='DNIdata2020PA', usecols=[5], skiprows=2)
...
def initRoutes(routeNums, numDays, eB_max, pCB_max):
    ...
    routeData = pd.read_excel('allRoutes.xlsx', usecols=[1, 2, 3, 4], index_col=0)
    # print(routeData)
