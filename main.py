...
generated = sio.loadmat('assignments.mat')

# Extract the 'assignment' matrix
assignment_matrix = generated['assignment']

assignment = m.addVars(B, D, R, vtype=gp.GRB.BINARY, name="assignment")

m.addConstrs(assignment[b,d,r] == assignment_matrix[b,d,r]
             for b in range(assignment_matrix.shape[0])
             for d in range(assignment_matrix.shape[1])
             for r in range(assignment_matrix.shape[2]))
...
m.addConstrs(sum(chargerUse[b, t] for t in tDay[d]) >= 2 * charging[b, d] for b in range(B) for d in range(D))
...
# create pandas dataframes for each variable
